                ...
                async getTemplates() {
                    ...
                    try {
                        responseData = await this.helpers.requestWithAuthentication.call(this, 'cellystialApi', {
                            method: 'GET',
                            url: `${constants_1.CELLYSTIAL_API_BASE_URL}/api/v1/integration/templates`,
                            qs: {
                                limit: 1000,
                            },
                            json: true,
                    ...
                    if (!Array.isArray(responseData)) {
                        throw new n8n_workflow_1.NodeOperationError(this.getNode(), `Expected an array of templates, but received: ${typeof responseData}`);
                    }
    ...
    async execute() {
        ...
        if (operation === 'generatePdfBatch') {
            ...
            for (let i = 0; i < items.length; i++) {
                const row = this.getNodeParameter('rowData', i);
                let data;
                if (typeof row === 'string') {
                    try {
                        ...
                        throw new n8n_workflow_1.NodeOperationError(this.getNode(), `Item ${i + 1}: "Row Data" is not valid JSON.`, { itemIndex: i });
                    }
                }
                else {
                    data = row;
                }
                const documentId = (this.getNodeParameter('documentId', i, '') || '').trim();
            ...
            if (withId === 0) {
                ...
                body = {
                    templateId,
                    items: rows.map((r) => r.filename
                        ? { documentId: r.documentId, filename: r.filename, data: r.data }
                        : { documentId: r.documentId, data: r.data }),
                };
            }
            else {
                throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Set "Document ID" on every row or leave it blank on all of them — the keyed batch shape requires a unique Document ID for each item.');
            }
            if (webhookUrl) {
            ...
            try {
                const responseData = await this.helpers.requestWithAuthentication.call(this, 'cellystialApi', {
                    method: 'POST',
                    url: `${constants_1.CELLYSTIAL_API_BASE_URL}/api/v1/generate/batch`,
                    body,
                    json: true,
                });
                ...
                throw new n8n_workflow_1.NodeOperationError(this.getNode(), error, { message: 'Cellystial batch generation failed.' });
            }
        }
        ...
        if (operation === 'getBatchStatus') {
            ...
            for (let i = 0; i < items.length; i++) {
                try {
                    ...
                    const responseData = await this.helpers.requestWithAuthentication.call(this, 'cellystialApi', {
                        method: 'GET',
                        url: `${constants_1.CELLYSTIAL_API_BASE_URL}/api/v1/generate/batch/${encodeURIComponent(batchId)}`,
                        json: true,
                    });
                    const results = Array.isArray(responseData === null || responseData === void 0 ? void 0 : responseData.results) ? responseData.results : [];
                    if (results.length > 0) {
                        // Emit one item per row so each generated PDF flows downstream mapped to its source row.
                        for (const row of results) {
                            statusData.push({
                                json: {
                                    batchId: (_a = responseData.id) !== null && _a !== void 0 ? _a : batchId,
        ...
        for (let i = 0; i < items.length; i++) {
            try {
                ...
                const fileName = this.getNodeParameter('fileName', i, 'document.pdf') || 'document.pdf';
                const binaryPropertyName = this.getNodeParameter('binaryPropertyName', i, 'data') || 'data';
                ...
                const responseData = await this.helpers.requestWithAuthentication.call(this, 'cellystialApi', {
                    method: 'POST',
                    url: `${constants_1.CELLYSTIAL_API_BASE_URL}/api/v1/generate`,
                    body: JSON.stringify({
                        templateId,
                        data: payload,
                    }),
                ...
                const err = error;
                let errorMessage = (_b = err.message) !== null && _b !== void 0 ? _b : 'PDF generation failed.';
